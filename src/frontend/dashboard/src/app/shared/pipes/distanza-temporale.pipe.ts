...
export class DistanzaTemporalePipe implements PipeTransform {
  ...
  transform(dataDa: Date, dataA: Date, formatoSintetico: string = "S"): string {
    ...
    let giorni, ore, minuti, secondi : number = 0;
    if ((!dataDa) || (!dataA) )
      return "";
    if (dataDa == dataA) 
      return "";
    ...
    if (formatoSintetico == "S") {
      ...
      if (ore <24 ) {
        ...
        if (ore == 0 && secondi != 0 ) { ritorno = ritorno + secondi +"s";}
        //ritorno = ritorno + moment(dataDa).diff(dataA);
        //ritorno = ritorno + ore +"h" +  minuti +"m" + secondi +"s" ;
      }
      else {
        giorni = moment(dataIniziale).diff(dataFinale, 'days');
        ritorno = ritorno + giorni +"d"; 
        ore = ore - (giorni*24);
        if (ore > 0) {
